...
async def analyze_replay(ctx, *args):
    ...
    # Create a dictionary of unique Pokemon names with their respective players
    unique_pokes = {}
    for player, poke_list in enumerate([pokes[:6], pokes[6:]], start=1):
        for poke in poke_list:
            if poke not in unique_pokes:
                unique_pokes[poke] = f"p{player}"

    # Initialize stats dictionary
    for poke in unique_pokes.keys():
        if poke not in stats:
            stats[poke] = {'player': unique_pokes[poke],
                           'kills': 0, 'deaths': 0}
