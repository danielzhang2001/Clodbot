...
async def sheet(ctx: commands.Context, *args: str):
    ...
    if command == "set":
        ...
        default_sheets_link[server_id] = remaining[0]
        await ctx.send(f"Default sheet link set to: {remaining[0]}")
        return
    server_id = ctx.guild.id if ctx.guild else 0
    if len(remaining) == 1 and server_id in default_sheets_link:
        sheets_link = default_sheets_link[server_id]
        data = remaining[0]
    elif len(remaining) == 2:
        sheets_link, data = remaining
    else:
        if server_id not in default_sheets_link:
            await ctx.send("No default sheet link set. Please specify a sheet link.")
            return
        await ctx.send(f"Incorrect number of arguments provided for {command}.")
        return
