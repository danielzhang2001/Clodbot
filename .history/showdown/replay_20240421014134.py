...
def testprocess_kills(json_data: dict) -> None:
    ...
    kills = faint_regex.findall(log)
    
    for fainted_player, fainted_pokemon in kills:
        fainted_pokemon = fainted_pokemon.strip()
        fainted_player_key = f"p{fainted_player}"
        # Search backwards from the faint event to find the action that caused it
        index = log.rfind(f"|faint|p{fainted_player}a: {fainted_pokemon}|")
        previous_actions = log[:index].split("\n")[::-1]
        for action in previous_actions:
            if "|move|" in action or "|switch|" in action:
                killer_match = re.search(r"\|p(\d)a: ([^\|]+)\|", action)
                if killer_match and killer_match.group(1) != fainted_player:
                    killer_player, killer_pokemon = killer_match.groups()
                    killer_pokemon = killer_pokemon.strip()
                    killer_player_key = f"p{killer_player}"
                    # Find the corresponding Pokémon entry in stats and increment kills
                    found = False
                    for poke, data in stats[killer_player_key].items():
