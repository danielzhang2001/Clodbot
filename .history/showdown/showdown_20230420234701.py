...
def process_kills(raw_data, stats, nickname_mapping_player1, nickname_mapping_player2):
    ...
    for faint in faints:
        if faint:
            ...
            player = match.group(1)
            fainted_pokemon = match.group(2)
            fainted_key = f"{player}: {nickname_mapping_player1.get(fainted_pokemon.strip(), fainted_pokemon.strip())}"
            if player == 'p1' else f"{player}: {nickname_mapping_player2.get(fainted_pokemon.strip(), fainted_pokemon.strip())}"
