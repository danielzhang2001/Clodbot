...
def testget_winner(json_data: dict) -> str:
...
def testget_loser(json_data: dict) -> str:
    # Retrieves the losing player.
...
def testget_difference(stats: dict, winner: str) -> str:
    ...
    if winner == "p1":
        difference = f"({player2_deaths - player1_deaths}-0)"
    else:
        difference = f"({player1_deaths - player2_deaths}-0)"
    return difference


def testinitialize_stats(pokemon_data: dict) -> dict:
...
def testprocess_faints(json_data: dict) -> None:
    # Updates stats with the death values for each Pokemon.
    log = json_data.get("log", "")
    fainted_regex = re.compile(r"\|faint\|p(\d)a: ([^\|]+)\n")
    for match in fainted_regex.finditer(log):
...
def testprocess_kills(json_data: dict) -> None:
    # Populates the kill values for all Pokemon based on the Pokemon on the opposing side when a Pokemon faints in the log.
    log = json_data.get("log", "")
    # Find all "faint" events and extract the details of the fainted Pokemon
    faint_events = re.findall(r"\|faint\|p(\d)a: ([^\|]+)\|", log)
    print(f"faint events: {faint_events}")
    for fainted_event in faint_events:
        print(f"fainted_event: {faint_event}")
        fainted_player, fainted_pokemon = fainted_event
        print(f"faint player and pokemon: {faint_player} {faint_pokemon}")
        fainted_pokemon = fainted_pokemon.strip()
        # Find the index of the faint event to search backward for the killing move
        event_index = log.rfind(f"|faint|p{fainted_player}a: {fainted_pokemon}|")
        # Examine log entries before the faint event
        log_slice = log[:event_index].split("\n")[::-1]
        for action in log_slice:
            # Match the action of the opposite player that caused the faint
            if (
                "|move|" in action or "|switch|" in action
            ) and f"p{fainted_player}a:" not in action:
                killer_match = re.search(r"\|p(\d)a: ([^\|]+)\|", action)
                if killer_match:
                    killer_player, killer_pokemon = killer_match.groups()
                    killer_pokemon = killer_pokemon.strip()
                    killer_key = f"p{killer_player}: {killer_pokemon}"
                    # Increment the kill count for the killer Pokemon found
                    if killer_key in stats and "kills" in stats[killer_key]:
                        stats[killer_key]["kills"] += 1
                        break  # Stop once the responsible Pokemon is found
    print(f"stats after kills: {stats}")
