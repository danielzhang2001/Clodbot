...
def testprocess_faints(json_data: dict) -> None:
    ...
    log = json_data.get("log", "")
    fainted_regex = re.compile(r"\|p(\d)a: ([^\|]+)\|0 fnt")
    for match in fainted_regex.finditer(log):
        player, fainted = match.groups()
        fainted = fainted.strip()
        player_key = f"p{player}"
        found = False
        for pokemon, data in stats[player_key].items():
            if data["nickname"] == fainted:
                data["deaths"] += 1
                found = True
                break
    print(f"stats after faints: {stats}")


def testprocess_kills(json_data: dict) -> None:
    # Populates the kill values for all Pokemon based on the Pokemon on the opposing side when a Pokemon faints in the log.
    log = json_data.get("log", "")
    kill_events = re.findall(r"(\|p(\d)a: ([^\|]+)\|0 fnt)", log)
    print(f"kill events: {kill_events}")
    for event_text, killer_player, killer_pokemon in kill_events:
        print(f"event text: {event_text}")
        print(f"killer player: {killer_player}")
        print(f"killer pokemon: {killer_pokemon}")
        index = log.rfind(event_text)
        previous_actions = log[:index].split("\n")[::-1]
        for action in previous_actions:
            if "|switch|" in action or "|move|" in action:
                match = re.search(r"\|p(\d)a: ([^\|]+)\|", action)
                print(f"match: {match}")
                if match and match.group(1) != killer_player:
                    _, opponent_pokemon = match.groups()
                    opponent_pokemon = opponent_pokemon.strip()
                    # Retrieve player and normalized Pokemon name for the killer
                    player_key = f"p{killer_player}"
                    real_name = stats[player_key][killer_pokemon][
                        "nickname"
                    ]  # Using nickname to get real name might be incorrect here
                    stats[player_key][real_name]["kills"] += 1
                    break
