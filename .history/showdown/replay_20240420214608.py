        ...
        for action in previous_actions:
            if "|switch|" in action or "|move|" in action:
                ...
                if match and match.group(1) != killer_player:
                    ...
                    killer_details = match.groups()
                    print(f"killer details: {killer_details}")
                    killer_key = f"p{killer_details[0]}: {stats['p'+killer_details[0]][killer_details[1]]['nickname']}"
                    print(f"killer key: {killer_key}")
                    if killer_key in stats:
                        stats[killer_key]["kills"] += 1
                    break
