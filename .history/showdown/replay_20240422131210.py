...
def testget_difference(winner: str, revives: List[Tuple[str, str]]) -> str:
...
def testcreate_message(plwinner: str, loser: str, difference: str) -> str:
    # Creates and returns final message.
    message = f"**Winner: ||{winner} {difference} {loser}||**\n\n"
    print(f"stats: {stats}")
    print(f"stats items: {stats.items()}")
    for player, pokemon_data in stats.items():
        print(f"player: {player}")
        print(f"pokemon_data: {pokemon_data}")
        message += f"{player}'s Pokemon:\n"
        stats_message = ""
        for pokemon, data in pokemon_data.items():
            kills = data["kills"]
            deaths = data["deaths"]
            stats_message += f"{pokemon} (Kills: {kills}, Deaths: {deaths})\n"
        message += f"||```\n{stats_message.strip()}\n```||\n"
    return message
