...
def testprocess_kills(json_data: dict) -> None:
    # Populates the kill values for all Pokemon based on the Pokemon on the opposing side when a Pokemon faints in the log.
    log = json_data.get("log", "")
    fainted_regex = re.compile(r"\|p(\d)a: ([^\|]+)\|.*?\|faint")
    for match in fainted_regex.finditer(log):
        fainted_player, fainted_pokemon = match.groups()
        fainted_pokemon = fainted_pokemon.strip()
        previous_actions = log[:match.start()].split("\n")[::-1]  # Search backwards from the fainted point
        for action in previous_actions:
            if "|switch|" in action or "|move|" in action and "p{}".format('2' if fainted_player == '1' else '1') in action:
                ...
                if killer_match:
                    ...
                    # Find the killer in stats and increment kills
