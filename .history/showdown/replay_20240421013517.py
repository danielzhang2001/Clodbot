...
def testprocess_kills(json_data: dict) -> None:
    # Populates the kill values for all Pokemon based on the Pokemon on the opposing side when a Pokemon faints in the log.
    log = json_data.get("log", "")
    # Find all faint events and the immediately preceding actions
    faint_regex = re.compile(r"(\|p(\d)a: ([^\|]+)\|faint)")
    for match in faint_regex.finditer(log):
        text, fainted_player, fainted_pokemon = match.groups()
        fainted_pokemon = fainted_pokemon.strip()
        # Find the index of the faint event and extract previous actions
        index = log.rfind(text)
        previous_actions = log[:index].split("\n")[::-1]
        for action in previous_actions:
            if ("|p1a:" in action or "|p2a:" in action) and match.group(
                2
            ) not in action:
                ...
                if killer_match:
                    ...
                    # Iterate through nicknames to find the killer and update kills
                    for poke, data in stats[player_key].items():
                        if data["nickname"] == killer_pokemon:
                            stats[player_key][poke]["kills"] += 1
                            break
                    break
