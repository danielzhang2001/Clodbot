...
def get_nickname_mappings(raw_data):
    ...
    for player, nickname, pokemon in switches + replaces:
        ...
        # Check if the nickname already exists, if so append to it, otherwise create a new list
        if nickname_stripped in nickname_mapping:
            nickname_mapping[nickname_stripped].append(actual_name)
        else:
            nickname_mapping[nickname_stripped] = [actual_name]
