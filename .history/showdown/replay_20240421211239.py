...
def testget_difference(stats: dict, winner: str) -> str:
    ...
    revive_counts = {}
    for player, pokemon in testget_revives():
        if player in stats:
            if pokemon in stats[player]:
                revive_counts.setdefault((player, pokemon), 0)
                revive_counts[(player, pokemon)] += 1
    for (player, pokemon), count in revive_counts.items():
        if pokemon in stats[player]:
            stats[player][pokemon]["deaths"] -= count
