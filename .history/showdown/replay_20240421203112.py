...
def testget_revives(json_data: dict) -> list:
    # Retrieves a pair of player and Pokemon for each Pokemon that was revived.
    revives = []
    log = json_data.get("log", "")
    revive_regex = re.compile(
        r"\|p(\d)a: ([^\|]+)\|Revival Blessing[^\n]+\n([^\n]*\n)*?\-heal\|p(\d): ([^\|]+)"
    )
    for match in revive_regex.finditer(log):
        player = match.group(4)
        pokemon = match.group(5).strip()
    ...
    for match in faint_regex.finditer(log):
        ...
        for action in actions:
            ...
            if killer and killer.group(1) != fainted_player:
                ...
                print(f"{killer_pokemon} killed {fainted_pokemon}!")
