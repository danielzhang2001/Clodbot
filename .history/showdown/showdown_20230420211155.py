...
def format_results(winner, difference, stats, players):
    ...
    for player_num, player_name in players.items():
        ...
        player_pokes = {key: value for key,
                        value in stats.items() if value['player'] == player_num}

        for index, (_, stat) in enumerate(player_pokes.items(), start=1):
            message += f"Pokemon {index}: {stat['poke']}\nKills: {stat['kills']}, Deaths: {stat['deaths']}\n\n"
