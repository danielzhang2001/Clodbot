...
def testprocess_kills(json_data: dict) -> None:
    ...
    for event_text, fainted_player, fainted_pokemon in kill_events:
        fainted_pokemon = fainted_pokemon.strip()
        index = log.rfind(event_text)
        previous_actions = log[:index].split('\n')[::-1]  # Reversed log up to the fainted event
        for action in previous_actions:
            if "|switch|" in action or "|move|" in action:
                match = re.search(r"\|p(\d)a: ([^\|]+)\|", action)
                if match and match.group(1) != fainted_player:  # Ensuring the killer is the opposite player
                    killer_player, killer_pokemon = match.groups()
                    ...
                    for pokemon, data in stats[player_key].items():
                        if data["nickname"] == killer_pokemon:
                            ...
                            break
                    if not found:
                        print(f"Killer Pokémon {killer_pokemon} not found in stats.")
                    break
