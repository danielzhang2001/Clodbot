...
async def give_set(ctx: commands.Context, *args: str) -> None:
    ...
    if input_str.lower().startswith("random"):
        ...
        if invalid_parts:
            await ctx.send(
                f"Too many fields provided for {', '.join(invalid_parts)}. Please provide at most a Pokemon, Generation, and Format."
            )
        await GiveSet.set_prompt(ctx, requests)


@bot.command(name="update", aliases=["delete"])
async def manage_sheet(ctx: commands.Context, *args: str):
    ...
    sheets_link, content = args
    creds = authenticate_sheet()
    if command == "update":
        message = await Update.update_sheet(creds, sheets_link, content)
    elif command == "delete":
        message = await Delete.delete_player(creds, sheets_link, content)
    await ctx.send(message)
