...
def update_buttons(message, button_id: str, deselected: bool, multiple: bool) -> None:
    ...
    view = ui.View()
    clicked_pokemon, *rest = button_id.rsplit("_", 4)[0:3]
    clicked_row = "_".join(clicked_pokemon)
    for component in message.components:
        for item in component.children:
            item_pokemon, *item_rest = item.custom_id.rsplit("_", 4)[0:3]
            item_row = "_".join(item_pokemon)
            same_row = item_row == clicked_row
            disabled = False
            if (
                multiple and same_row and item.label == "Select"
            ):  # Assuming the first button's label is "Select"
            ...
            # Update the button with the determined style and disabled state
