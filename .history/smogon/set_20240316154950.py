...
def update_buttons(view: ui.View, button_id: str) -> None:
    ...
    view = ui.View()  # Create a new View to hold your updated buttons
    for set_name in set_names:
        btn_id = f"unique_identifier_for_{set_name}"  # Construct the button ID in the same way it was originally done
        style = (
            ButtonStyle.success
            if set_name == selected_set_name
            else ButtonStyle.secondary
        )
        button = ui.Button(label=set_name, custom_id=btn_id, style=style)
        view.add_item(button)
    return view
