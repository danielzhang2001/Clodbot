...
def update_buttons(message, button_id: str, deselected: bool, multiple: bool) -> None:
    ...
    view = ui.View()
    clicked_pokemon, clicked_generation, clicked_format, *rest = button_id.rsplit(
        "_", 4
    )
    clicked_identifier = f"{clicked_pokemon}_{clicked_generation}_{clicked_format}"
    print(f"CLICKED IDENTIFIER IS: {clicked_identifier}")
    for component in message.components:
        first_button = True
        for item in component.children:
            item_pokemon, item_generation, item_format, *item_rest = (
                item.custom_id.rsplit("_", 4)
            )
            item_identifier = f"{item_pokemon}_{item_generation}_{item_format}"
            same_row = item_identifier == clicked_identifier
            disabled = False

            if multiple and same_row and first_button:
