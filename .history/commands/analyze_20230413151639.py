...
class Analyzer:
    ...
    async def analyze_replay(replay_link):
        ...
        for nickname, pokemon in switches:
            if nickname.startswith('p1'):
                match = re.search(r"\|switch\|p1a: (.*?)(\||,)", nickname)
                if match:
                    nickname_mapping_player1[nickname.strip()] = match.group(1)
                else:
                    nickname_mapping_player1[nickname.strip(
                    )] = pokemon.strip()
            elif nickname.startswith('p2'):
                match = re.search(r"\|switch\|p2a: (.*?)(\||,)", nickname)
                if match:
                    nickname_mapping_player2[nickname.strip()] = match.group(1)
                else:
                    nickname_mapping_player2[nickname.strip(
                    )] = pokemon.strip()
