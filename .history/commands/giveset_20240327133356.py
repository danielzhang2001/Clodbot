...
class GiveSet:
    ...
    async def set_prompt(
        ...
        key = str(uuid.uuid4())
        prompt = "Please select a set type for the following Pokemon: "
        tasks = [
            GiveSet.fetch_set_names(req["pokemon"], req["generation"], req["format"])
            for req in requests
        ]
        results = await asyncio.gather(*tasks)
        await ctx.send(prompt)
        request_count = len(requests)
        for index, (request, set_names) in enumerate(zip(requests):
            view = View()
