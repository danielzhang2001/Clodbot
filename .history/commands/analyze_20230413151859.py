...
class Analyzer:
    ...
    async def analyze_replay(replay_link):
        ...
        # Extract the nickname and Pokémon name from the switch line
        match = re.search(r"\|switch\|" + nickname + r": (.*?)\|.*", raw_data)
        if match:
            nickname_mapping[match.group(1).strip()] = re.sub(
                r', .*', '', pokemon.strip())
        else:
            nickname_mapping[pokemon.strip()] = pokemon.strip()
