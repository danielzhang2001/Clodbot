...
class GiveSet:
    ...
    def fetch_set(pokemon: str, generation: str, format_name: str, set_name: str) -> str:
...
def format_set(moveset: dict) -> str:
    ...
    evs = "/".join([f'{value} {key.upper()}' for key, value in moveset.get("evconfigs", [{}])[0].items()])
    nature = moveset.get("natures", [])[0] if moveset.get("natures") else "None"
    moves = "\n- ".join([move["move"] for move in moveset.get("moveslots", []) for move in move])
