...
class ManageSheet:
    ...
    async def update_sheet(
        ...
        if not sheet.is_valid_sheet(creds, sheet_link):
            creds = sheet.authenticate_sheet(force_login=True)
            if not sheet.is_valid_sheet(creds, sheet_link):
        ...
        stats = replay.get_stats(json_data)
        ...
        for player, pokemon_data in stats.items():
            player = replay.get_players(json_data)[player]
            ...
            if sheet.check_labels(values, player_name):
    ...
    async def delete_player(
        ...
        values = result.get("values", [])
        players = [player[0] for player in sheet.get_players(values)]
        ...
        sheet.delete_data(service, spreadsheet_id, sheet_id, section_range)
    ...
    async def list_data(creds: Credentials, sheet_link: str, data: str) -> str:
        ...
        values = result.get("values", [])
        if data.lower() == "players":
            if not sheet.get_players(values):
                raise NoPlayers()
            return sheet.create_player_message(values)
        elif data.lower() == "pokemon":
            if not get_pokemon(values):
                raise NoPokemon()
            return sheet.create_pokemon_message(values)
