...
class Analyze:
    ...
    async def test_replay(replay_link: str) -> str:
        ...
        nickname_mapping1, nickname_mapping2 = testget_nickname_mappings(json_data)
        ...
        stats = testprocess_revives(raw_data, stats)
        winner = testget_winner(raw_data)
        loser = testget_loser(raw_data)
        difference = testget_difference(raw_data, players)
        message = testcreate_message(winner, loser, difference, stats, players)
        return message
