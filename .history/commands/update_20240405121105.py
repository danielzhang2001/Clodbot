...
class Update:
    ...
    def next_cell(values, column_letters):
        ...
        column_indices = {"B": 0, "D": 1, "F": 2, "H": 3}
        column_letters = ["B", "D", "F", "H"]

        for row_index, row in enumerate(values):
            # row might not be fully populated, check each column one by one
            for col_index, col_letter in enumerate(column_letters):
                try:
                    # Check if the cell in this column is empty
                    if row[col_index] == "":
                        return f"{col_letter}{row_index + 2}"
                except IndexError:
                    # This means the row is not fully populated
                    return f"{col_letter}{row_index + 2}"

        # If all rows are filled, place it in the next new row at the first column
        return f"B{row_count + 2}"
