        ...
        try:
            ...
            for name in player_names:
                if name not in existing_names:
                    ...
                    merge_start = next_cell(values)
                    merge_end = next_cell(values)[0] + str(
                        ...
                        "requests": [
                            {
                                "mergeCells": {
                                    "range": {
                                        "sheetId": sheets_id,  # You need the specific ID of the 'Stats' sheet here
                                        "startRowIndex": int(cell_range[1:])
                                        - 1,  # Convert 1-based index to 0-based
                                        "endRowIndex": int(
                                            merge_end[1:]
                                        ),  # Convert end cell 1-based index to 0-based
                                        "startColumnIndex": ord(cell_range[0])
                                        - ord("A"),  # Convert 'B' to index 1
