...
class GiveSet:
    ...
    async def fetch_set(
        ...
        try:
            ...
            url_base = f"https://www.smogon.com/dex/{gen_code if gen_code else 'ss'}/pokemon/{pokemon.lower()}/"

            if format:
                url = f"{url_base}{format.lower()}/"
                if not (
                    is_valid_pokemon(driver, pokemon)
                    and is_valid_format(driver, format)
                ):
                    return None, None
            else:
                url = url_base
