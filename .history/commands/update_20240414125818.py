...
class Update:
    ...
    async def update_sheet(
        ...
        try:
            if "docs.google.com/spreadsheets/" not in sheets_link or "/d/" not in sheets_link:
            valid = False
        else:
            parts = sheets_link.split("/d/")
            spreadsheet_id = parts[1].split("/")[0] if len(parts) > 1 else ''
            valid = bool(spreadsheet_id)
        if not valid:
            raise Exception("**{sheets_link}** is an invalid Google Sheets link.")
        except Exception:
