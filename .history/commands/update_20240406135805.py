...
class Update:
    ...
    def next_cell(values):
        ...
        for row_index, row = enumerate(values):
            for group_index, in range(0, len(row), 4)
            group = row[group_index:group_index + 3]
            if any (cell == "" for cell in group):
                col = chr(66 + (group_index // 4) * 2)  # Calculate based on group index
                row = (row_index * 14) + 2  # Real row number, accounting for the 14-row jump pattern
                return col, row
