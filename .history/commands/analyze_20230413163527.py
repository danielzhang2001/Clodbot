...
class Analyzer:
    ...
    async def analyze_replay(replay_link):
       ...
       # Find all lines when a Pokemon is switched in
        ...
        # Map all Pokemon in stats to their nickname_mapping
        for stat in stats.values():
            if stat['player'] == 'p1':
                ...
                nickname_mapping = nickname_mapping_player2
                player = 'p2'
            old_player_poke = f"p{player}: {stat['poke']}"
            stat['poke'] = nickname_mapping.get(stat['poke'], stat['poke'])
            new_player_poke = f"p{player}: {stat['poke']}"
            stat[old_player_poke] = new_player_poke
