...
class Analyzer:
    ...
    async def analyze_replay(replay_link):
        ...
        for nickname, pokemon in nickname_mapping_player2.items():
            print(f"{nickname}: {pokemon}")
        mapped_pokes = [nickname_mapping.get(poke, poke) for poke in pokes]
        print("MAP pokes:")
        for mappoke in mapped_pokes:
            print(mappoke)

        # Initialize stats dictionary
        for player, poke_list in enumerate([mapped_pokes[:6], mapped_pokes[6:]], start=1):
