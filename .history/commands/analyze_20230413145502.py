...
class Analyzer:
    ...
    async def analyze_replay(replay_link):
        ...
        for player, nickname in switches:
            if player == 'p1':
                nickname_mapping = nickname_mapping_player1
            else:
                nickname_mapping = nickname_mapping_player2

            nickname_mapping[nickname.strip()] = re.sub(
                r', .*', '', nickname.strip())

        print(f"Player 1 nickname mapping: {nickname_mapping_player1}")
        print(f"Player 2 nickname mapping: {nickname_mapping_player2}")
