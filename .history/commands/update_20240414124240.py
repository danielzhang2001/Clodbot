...
class Update:
    ...
    async def update_sheet(
        ...
        try:
            if "docs.google.com/spreadsheets/" not in sheets_link:
                raise ValueError("Invalid URL format for Google Sheets.")
            parts = sheets_link.split("/d/")
            if len(parts) < 2:
                raise ValueError("No document ID found in the URL.")
            sheets_id = parts[1].split("/")[0]
            if not sheets_id:
                raise ValueError("Invalid document ID extracted.")
        except Exception as e:
            await ctx.send(
                f"**{sheets_link}** is an invalid Google Sheets link. {str(e)}"
            )
        return
        try:
            spreadsheet_id = sheets_link.split("/d/")[1].split("/")[0]
        except (IndexError, Exception):
            return f"**{sheets_link}** is an invalid Google Sheets link."
