...
class GiveSet:
    ...
    async def set_selection(
        ...
        if formatted_sets:
            formatted_sets = f"```\n{formatted_sets}\n```"
        if multiple:
            initial_button_row_id = GiveSet.initial_prompt.get(channel_id)
            if initial_button_row_id:
                initial_button_row_message = await interaction.channel.fetch_message(
                    initial_button_row_id
                )
                formatted_sets = "\n\n".join(
                    [
                        f"{data}"
                        for _, data in GiveSet.selected_sets.get(message_id, {}).items()
                    ]
                )
                if formatted_sets:
                    formatted_sets = f"```\n{formatted_sets}\n```"
                if message_id == initial_button_row_id:
                    current_content = initial_button_row_message.content
                    updated_content = (
                        f"{formatted_sets}\n{current_content}"
                        if current_content
                        else formatted_sets
                    )
                    await initial_button_row_message.edit(content=updated_content)
                    view = update_buttons(
                        initial_button_row_message,
