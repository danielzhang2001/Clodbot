       ...
       # Replace all nicknames with the actual Pokemon names for both players
        ...
        for faint in faints:
            if faint:
                ...
                # Decrement the death counter if the Pokemon has been revived
                for revive in revives:
                    if fainted_key in revive:
                        match = re.search(
                            r'\|move\|(p\d)a: (.*?)\|Revival Blessing\|', revive)
                        reviving_player = match.group(1)
                        revived_pokemon = match.group(2)
                        revived_key = f"{reviving_player}: {nickname_mapping_player1.get(revived_pokemon.strip(), revived_pokemon.strip())}" if reviving_player == 'p1' else f"{reviving_player}: {nickname_mapping_player2.get(revived_pokemon.strip(), revived_pokemon.strip())}"
