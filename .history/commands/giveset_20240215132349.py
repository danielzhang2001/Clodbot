    ...
    async def set_prompt(ctx, pokemon_data):
        ...
        if len(pokemon_data) > 1:
            ...
            for pokemon, sets, url in pokemon_data:
                ...
                message = await ctx.send(content=prompt, view=view)
                GiveSet.awaiting_response[unique_id]["views"][message.id] = view
                GiveSet.awaiting_response[unique_id]["message_ids"].append(
                    message.id
            ...
            for pokemon, sets, url in pokemon_data:
                ...
                views[message.id] = view
        message = await ctx.send(prompt.strip(), view=view)
        # Now you can safely set the view for this message
        GiveSet.awaiting_response[unique_id]["views"][message.id] = view
        # And add the message ID to the list of message_ids
