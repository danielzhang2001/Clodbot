...
class Update:
    ...
    async def update_sheet(creds, sheets_id, replay_link):
        ...
        try:
            ...
            stats_sheet_exists = any(sheet["properties"]["title"] == "Stats" for sheet in sheets)
            ...
            result = service.spreadsheets().values().get(spreadsheetId=sheets_id, range="Stats!B2:H1000").execute()
            all_values = result.get("values", [])
            values = filter_columns_and_rows(all_values)
            for name in player_names:
                if name not in set(cell for row in values for cell in row if cell):
                    col_letter, row_index = next_cell(values, ["B", "D", "F", "H"])
