...
class Update:
    ...
    def next_cell(values):
        ...
        for block_start in range(0, len(values), 15):
            ...
            for index, letter in enumerate(["B", "F", "J", "N"]):
                ...
                details_index = col_idx * 3
                group_cells = [
                    names_row[name_index] if len(names_row) > name_index else "",
                    details_row[name_index] if len(details_row) > name_index else "",
                    (
                        details_row[name_index + 1]
                        if len(details_row) > name_index + 1
                        else ""
                    ),
                    (
                        details_row[name_index + 2]
                        if len(details_row) > name_index + 2
                        else ""
                    ),
                ]
