...
class GiveSet:
    ...
    async def set_prompt(
        ...
        if request_count > 1:
            ...
            generation = get_gen(request.get("generation")) or "none"
            format = request.get("format", "none") or "none"
            prompt += f"**{pokemon.upper()}{f' {generation}' if generation else ''}{f' {format}' if format else ''}**"
        prompt += ":"
