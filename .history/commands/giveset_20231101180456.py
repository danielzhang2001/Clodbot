...
class GiveSet:
    ...
    async def fetch_set(pokemon: str, generation: str, format: str, set: str) -> str:
        """Fetch the set from Smogon for the given Pokemon name, generation, format, and set name. If only Pokemon given, assume most recent generation"""
        try:
            if generation is None:
                generation = 'gen9'
            if format is None:
                format = ''  # Assuming 'ou' is a valid format
            if set is None:
                set = 'default'  # You will need to decide how to handle a default set
