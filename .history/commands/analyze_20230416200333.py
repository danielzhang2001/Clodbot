...
class Analyzer:
    ...
    async def analyze_replay(replay_link):
       ...
       # Replace all nicknames with the actual Pokemon names for both players
        ...
        revives = [line for line in raw_data.split(
            '\n') if '|move|' in line and 'Revival Blessing' in line]
        ...
        for faint in faints:
            if faint:
                ...
                for revive in revives:
                    if revive:
                        revive_match = re.search(
                            r'\|move\|(p\d)a: (.*?)\|Revival Blessing\|p\d:a? (.+?)\|', revive)
                        print(f"REVIVED MATCH: {revive_match}")
                        revived_player = revive_match.group(1)
                        print(f"REVIVED PLAYER: {revived_player}")
                        revived_pokemon = revive_match.group(2)
                        print(f"REVIVED POKEMON: {revived_pokemon}")
                        revived_key = f"{revived_player}: {nickname_mapping_player1.get(revived_pokemon.strip(), revived_pokemon.strip())}" if revived_player == 'p1' else f"{revived_player}: {nickname_mapping_player2.get(revived_pokemon.strip(), revived_pokemon.strip())}"
                        print(f"Revived: {revived_key}")

                        # Decrement the death counter
                        if revived_key in stats:
                            stats[revived_key]['deaths'] -= 1
                        break
