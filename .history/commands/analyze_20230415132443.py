        ...
        for faint in faints:
            if faint:
                ...
                fainted_pokemon = re.search(
                    r'\|faint\|(p\d)a: (.*[^|])', faint).groups()
                fainted_player = match.group(1)
                fainted_pokemon = match.group(2)
                fainted_key = f"{fainted_player}: {nickname_mapping_player1.get(fainted_pokemon.strip(), fainted_pokemon.strip())}" if player == 'p1' else f"{player}: {nickname_mapping_player2.get(fainted_pokemon.strip(), fainted_pokemon.strip())}"
