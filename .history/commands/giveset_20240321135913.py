...
class GiveSet:
    ...
    async def set_selection(
        ...
        if GiveSet.selected_states.get(message_id) == new_state:
            ...
            GiveSet.selected_sets.get(message_id, {}).pop(pokemon, None)
        else:
        ...
        if formatted_sets:
            formatted_sets = f"```\n{formatted_sets}\n```"
        if multiple:
            initial_message_id = GiveSet.initial_prompt.get(interaction.channel.id)
            if initial_message_id:
                initial_message = await interaction.channel.fetch_message(
                    initial_message_id
                )
                current_content = (
                    initial_message.content.split("```")[0]
                    if "```" in initial_message.content
        ...
        view = update_buttons(
            ...
            GiveSet.selected_states[message_id] is None,
            multiple,
        )
        await interaction.edit_original_response(content=formatted_sets, view=view)
