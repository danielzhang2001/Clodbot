...
class GiveSet:
    ...
    async def set_selection(
        ...
        print(f"CURRENT STATE: {GiveSet.selected_states.get(key)}")
        print(f"CURRENT STATE KEY: {key}")
        ...
        if deselected:
            ...
            print(f"KEY {key} ASSIGNED TO {new_state}")
            GiveSet.selected_states[key] = new_state
            GiveSet.selected_sets.setdefault(key, {})[pokemon_state] = [set_data]
            if key in GiveSet.selected_states:
                if pokemon_state in GiveSet.selected_states[key]:
                    GiveSet.selected_states[key][pokemon_state] = new_state
                else:
                    GiveSet.selected_states[key][pokemon_state] = new_state
            else:
                GiveSet.selected_states[key] = {pokemon_state: new_state}
