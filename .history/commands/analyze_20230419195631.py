...
class Analyze:
    ...
    async def analyze_replay(replay_link):
       ...
       # Replace all nicknames with the actual Pokemon names for both players
        ...
        # Find all lines when a Pokemon is switched in or replaced by Zoroark
        switches = re.findall(
            r"\|switch\|(p\d)a: (.*?)(?:\||, )(.+?)\|", raw_data)
        replaces = re.findall(
            r"\|replace\|(p\d)a: (.*?)(?=\||$)(?:\|)(.*[^|\n])", raw_data)

        # Update the nickname mapping for both players
        for player, nickname, pokemon in switches + replaces:
            update_mapping(nickname_mapping, player, nickname, pokemon)


        # Replace all nicknames with the actual Pokemon names for both players
mapped_pokes_player1 = [nickname_mapping['p1'].get(
    poke, poke) for poke in pokes[:p1_count]]
mapped_pokes_player2 = [nickname_mapping['p2'].get(
    poke, poke) for poke in pokes[p1_count:]]
