...
def create_player_message(values: List[List[str]]) -> str:
    ...
    # Format the message with each name on a new line, and proper Markdown formatting
    message = "**PLAYER NAMES:**\n"  # Start with the bold title and a newline
    message += "```\n"  # Start of the code block on a new line
    message += "\n".join(
        names
    )  # Join all names with a newline character to list them one per row
    message += (
        "\n```"  # End of the code block with a newline before the closing backticks
    )
