...
def create_player_message(values: List[List[str]]) -> str:
    ...
    # Sort players: first by kills (descending), then by deaths (ascending) in case of tie
    sorted_players = sorted(players, key=lambda x: (-int(x[1]), int(x[2])))
...
def get_players(values: List[List[str]]) -> List[List[str]]:
    ...
    for index, name in enumerate(header_row):
        ...
        total_kills = 0
        total_deaths = 0
        for i in range(2, min(14, len(values))):
