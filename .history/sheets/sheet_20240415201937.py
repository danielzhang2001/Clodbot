...
def clear_text(service: Resource, spreadsheet_id: str, sheet_id: int, cell_range: str):
    # Clears all text within the specified range in a Google Sheet.
    _, cell_range = cell_range.split("!")
    start_cell, end_cell = cell_range.split(":")
    start_row = int("".join(filter(str.isdigit, start_cell))) - 1
    end_row = int("".join(filter(str.isdigit, end_cell)))
    ...
    requests = [
        {
            "updateCells": {
                ...
                "fields": "userEnteredValue",  # Specifies that only the values should be cleared.
                "rows": [
                    {
                        "values": [
                            {
                                "userEnteredValue": None
                            }  # Clear the text in each cell in the range.
                        ]
                        * (
                            end_col - start_col
                        )  # Repeat clearing for each column in the range.
                    }
                ]
                * (end_row - start_row),
            }
