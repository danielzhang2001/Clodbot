...
_, range_part = cell_range.split('!')
    start_cell, end_cell = range_part.split(':')
    start_col, start_row = ''.join(filter(str.isalpha, start_cell)), int(''.join(filter(str.isdigit, start_cell)))
    end_col, end_row = ''.join(filter(str.isalpha, end_cell)), int(''.join(filter(str.isdigit, end_cell)))
    
    # Convert column letters to zero-based index
    start_col_index = sum((ord(char) - ord('A') + 1) * 26 ** i for i, char in enumerate(reversed(start_col))) - 1
    end_col_index = sum((ord(char) - ord('A') + 1) * 26 ** i for i, char in enumerate(reversed(end_col))) - 1

    # Prepare the request body for the batchUpdate API method
    body = {
        "requests": [
            {
                "mergeCells": {
    ...
    # Execute the request using the Google Sheets API
    service.spreadsheets().batchUpdate(spreadsheetId=spreadsheet_id, body=body).execute()


def bold_cells(
    service: Resource, spreadsheet_id: str, sheet_id: str, cell_range: str
) -> None:
    # Bolds all the cells for each player's section for formatting purposes.
    body = {
        "requests": [
            {
                "repeatCell": {
                    "range": {
                        ...
                        - 1,
                        "endRowIndex": int(
                            cell_range.split(":")[1].lstrip(
                                "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                            )
                        ),
                        "startColumnIndex": ord(cell_range.split("!")[1][0]) - ord("A"),
                        "endColumnIndex": ord(cell_range.split(":")[1][0])
                        - ord("A")
                        + 1,
                    },
