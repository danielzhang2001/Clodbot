...
def clear_cells(service: Resource, spreadsheet_id: str, sheet_id: int, cell_range: str):
    # Clears all formatting from a specified cell range in a Google Sheet.
    ...
    requests = [
        {
            "repeatCell": {
                ...
                "cell": {
                    "userEnteredFormat": {}  # Empty format effectively clears formatting
                },
                "fields": "userEnteredFormat",  # Specifies that only formatting should be cleared
            }
        }
    ]
    ...
    # Execute the request to clear formatting
    response = (
        service.spreadsheets()
        .batchUpdate(spreadsheetId=spreadsheet_id, body=body)
        .execute()
    )
    return response  # Returns the response for confirmation or debugging
