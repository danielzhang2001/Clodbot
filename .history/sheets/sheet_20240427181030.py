...
def update_data(
    ...
    pokemon = [row[0].strip() for row in values if row and row[0].strip()]
    start_col = cell_range.split("!")[1].split(":")[0][:-1]
    end_col = cell_range.split("!")[1].split(":")[1][:-2]
    start_row = int(cell_range.split("!")[1].split(":")[0][-1])
    end_row = int(cell_range.split("!")[1].split(":")[1][-1])
    for pokemon_name, stats in pokemon_data:
        if pokemon_name in pokemon:
            row_index = start_row + pokemon[pokemon_name]
    ...
    for pokemon_name, stats in pokemon_data:
        if pokemon_name not in pokemon_indices:
            # Find the first truly empty row at each addition
            empty_row_index = next(
                (start_row + idx for idx, row in enumerate(values) if not row or not any(cell.strip() for cell in row)),
                end_row + 1
            )
            empty_row_range = f"Stats!{start_col}{empty_row_index}:{end_col}{empty_row_index}"
            add_pokemon(
                service,
                spreadsheet_id,
                empty_row_range,
            ...
            # Update values to include the newly added row
            values.append([pokemon_name] + [None] * (len(values[0]) - 1))
...
def add_pokemon(
    ...
    print(f"ADD POKEMON RANGE: {row_range}")
...
def update_pokemon(
    ...
    print(f"UPDATE POKEMON RANGE: {row_range}")
    result = (
        service.spreadsheets()
        .values()
    ...
    values = result.get("values", [])
    games = int(values[1]) + 1
    kills = int(values[2]) + stats[0]
    deaths = int(values[3]) + stats[1]
    updated_values = [[values[0], games, kills, deaths]]
    service.spreadsheets().values().update(
        spreadsheetId=spreadsheet_id,
        range=row_range,
