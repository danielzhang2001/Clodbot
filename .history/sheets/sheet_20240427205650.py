...
def create_pokemon_message(values: List[List[str]]) -> str:
    ...
    pokemon = sorted(
        ...
        key=lambda x: (
            (x[2].strip() == "", -int(x[2]) if x[2].strip() != "" else 0),
            (x[3].strip() == "", int(x[3]) if x[3].strip() != "" else 0),
        ),
    )
    message = "**POKEMON:**\n```"
    message += "\n".join(
            ...
            for idx, value in enumerate(row):
                if value.strip() and not (
                    ...
                    if player and idx + 2 < len(row) and idx + 3 < len(row):
                        kills = row[idx + 2].strip() if idx + 2 < len(row) else "N/A"
                        deaths = row[idx + 3].strip() if idx + 3 < len(row) else "N/A"
