...
def get_pokemon(values: List[List[str]]) -> List[str]:
    ...
    for i in range(
        2, len(values), 15
    ):  # Starts from the third row, iterates over blocks of data
        for j in range(
            i, min(i + 12, len(values))
        ):  # Processes up to 12 rows of Pokémon data
            row = values[j]
            for idx, name in enumerate(row):
                if name.strip() and not (
                    ...
                    # Ensure there are enough elements to get 'kills' and 'deaths'
                    if idx + 2 < len(row) and idx + 3 < len(row):
                        kills = row[idx + 2].strip()
                        deaths = row[idx + 3].strip()
                        # Append the new list [name, kills, deaths] to pokemon_stats
                        pokemon_stats.append([name.strip(), kills, deaths])
