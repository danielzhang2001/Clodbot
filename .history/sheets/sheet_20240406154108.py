...
def next_cell(values):
    ...
    for section in range(0, len(values), 15):
        ...
        for index, letter in enumerate(letters):
            start_index = index * 4
            group_cells = [names_row[start_index] if len(names_row) > start_index else "",
                           details_row[start_index] if len(details_row) > start_index else "",
                           details_row[start_index + 1] if len(details_row) > start_index + 1 else "",
                           details_row[start_index + 2] if len(details_row) > start_index + 2 else ""]
