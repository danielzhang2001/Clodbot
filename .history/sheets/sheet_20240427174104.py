...
def update_data(
    ...
    current_pokemon = {
        row[0]: idx for idx, row in enumerate(existing_rows) if row
    }

    start_col = cell_range.split('!')[1].split(':')[0][:-1]  # Assuming range like "Stats!B5:E5"
    ...
    empty_row_range = f"Stats!{start_col}{empty_row_index}:{start_col[-1]}{empty_row_index}"
    for pokemon_name, stats in pokemon_data:
...
def add_pokemon(
    ...
    range = f"Stats!{col_range}{row}"
    print(f"add pokemon range: {range}")
...
def update_pokemon(
    ...
    range = f"Stats!{col_range}{row}"
    print(f"update pokemon range: {range}")
    result = (
