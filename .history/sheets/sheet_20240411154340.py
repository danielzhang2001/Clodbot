...
def update_data(
    ...
    for pokemon_name, new_stats in pokemon_data:
        if pokemon_name in current_pokemon:
            ...
            add_pokemon(
                sheet_name, start_col, end_col, row, pokemon_name, stats, updates
            )
    if updates:
...
def update_pokemon(
    ...
    pokemon_name: str,
    stats: List[int],
    updates: List[Dict[str, List[List[Union[str, int]]]]],
):
...
def add_pokemon(
    sheet_name, start_col, end_col, end_row, row, pokemon_name, stats, updates
):
    if row is None:
        row = end_row + 1
    new_kills, new_deaths = stats
    insert_range = f"{sheet_name}!{start_col}{row}:{end_col}{row}"
    updates.append(
        {
            "range": insert_range,
