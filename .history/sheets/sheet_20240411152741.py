    ...
    for pokemon_name, (new_kills, new_deaths) in pokemon_data:
        if pokemon_name in current_pokemon:
            ...
            update_pokemon(
                ...
                row_index,
                pokemon_name,
                [current_kills + stats[0], current_deaths + stats[1]],
                updates,
            )
        else:
            if empty_row is None:
                empty_row = end_row + 1
            add_pokemon(
                sheet_name, start_col, end_col, empty_row, pokemon_name, stats, updates
            )
            empty_row += 1
    if updates:
...
def update_pokemon(
    sheet_name, start_col, end_col, row_index, pokemon_name, new_stats, updates
):
    current_kills, current_deaths = new_stats
    updated_kills = current_kills
    updated_deaths = current_deaths
...
def add_pokemon(
    sheet_name, start_col, end_col, insert_row, pokemon_name, stats, updates
):
    new_kills, new_deaths = stats
