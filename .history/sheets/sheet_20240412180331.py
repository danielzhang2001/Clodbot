...
def bold_cells(service: Resource, spreadsheet_id: str, cell_range: str) -> None:
    # Applies bold formatting to all cells within the specified range.
    # The cell_range should be formatted as "SheetName!A1:B10".
    bold_body = {
        "requests": [
            {
                "repeatCell": {
                    "range": {
                        "sheetId": get_sheet_id(
                            service, spreadsheet_id, cell_range.split("!")[0]
                        ),
                        "startRowIndex": int(
                            cell_range.split("!")[1]
                            .split(":")[0]
    ...
    service.spreadsheets().batchUpdate(
        spreadsheetId=spreadsheet_id, body=bold_body
    ).execute()
