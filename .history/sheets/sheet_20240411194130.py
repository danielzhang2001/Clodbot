...
def update_data(
    ...
    for pokemon_name, new_stats in pokemon_data:
        if pokemon_name in current_pokemon:
            ...
            first_empty_row, end_row = add_pokemon(
                sheet_name=sheet_name,
                start_col=start_col,
                end_col=end_col,
                pokemon_name=pokemon_name,
                new_stats=[new_kills, new_deaths],
...
def add_pokemon(
    ...
    """
    Inserts a new Pokemon entry into the sheet.

    Args:
    sheet_name (str): The name of the Google Sheet.
    start_col (str): The starting column for the range.
    end_col (str): The ending column for the range.
    pokemon_name (str): The name of the Pokemon to add.
    new_stats (List[int]): A list containing the kills and deaths statistics.
    first_empty_row (Optional[int]): The first empty row available for insertion, or None if no empty row has been identified.
    end_row (int): The current last row of the data before insertion.
    updates (List[Dict[str, Any]]): The list to append update commands to.

    Returns:
    Tuple[Optional[int], int]: Returns the updated values of `first_empty_row` and `end_row`.
    """
    insert_row = first_empty_row if first_empty_row is not None else end_row + 1
    ...
    return first_empty_row, end_row
