...
range = f"{sheet_name}!{col_range}{row}"
    values = {
        "values": [[stats[0] + 1, stats[0], stats[1]]]  # Assumes columns are Games, Kills, Deaths and stats are [kills, deaths]
    }
    service.spreadsheets().values().update(spreadsheetId=spreadsheet_id, range=range, body=values, valueInputOption='USER_ENTERED').execute()
