...
def update_data(
    ...
    sheet_name, full_range = cell_range.split("!")
    col_range = "".join(filter(str.isalpha, full_range))
    start_row = int("".join(filter(str.isdigit, full_range.split(":")[0])))
    end_row = int("".join(filter(str.isdigit, full_range.split(":")[1])))
    result = (
        service.spreadsheets()
        .values()
        .get(
            spreadsheetId=spreadsheet_id, range=f"{sheet_name}!A{start_row}:A{end_row}"
        )
        .execute()
    )
    existing_rows = result.get("values", [[]] * (end_row - start_row + 1))
    current_pokemon = {
        row[0]: start_row + idx for idx, row in enumerate(existing_rows) if row
    }
