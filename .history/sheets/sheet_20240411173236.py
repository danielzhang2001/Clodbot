...
def update_data(
    ...
    for pokemon_name, new_stats in pokemon_data:
        if pokemon_name in current_pokemon:
            ...
            print("BEFORE")
            current_kills = safe_int(
                current_values[row_index - start_row][1]
                if len(current_values[row_index - start_row]) > 1
                else 0
            )
            current_deaths = safe_int(
                current_values[row_index - start_row][2]
                if len(current_values[row_index - start_row]) > 2
                else 0
            )
            print("AFTER")
