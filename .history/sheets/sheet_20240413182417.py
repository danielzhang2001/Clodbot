...
def clear_cells(service: Resource, spreadsheet_id: str, sheet_id: int, cell_range: str):
    # Clears all formatting in the range.
     _, cell_range = cell_range.split("!")
    start_cell, end_cell = cell_range_part.split(":")
    start_row = int("".join(filter(str.isdigit, start_cell))) - 1
    end_row = int("".join(filter(str.isdigit, end_cell)))
    start_col = ord(start_cell[0]) - ord("A")
    end_col = ord(end_cell[0]) - ord("A") + 1
    banding_ids = get_bandings(service, spreadsheet_id, sheet_id, start_row, end_row, start_col, end_col)
    requests = [
        {
            "repeatCell": {
                "range": {
...
def get_bandings(
    ...
    banded_ranges = result.get("sheets", [])[0].get("bandedRanges", [])
    overlapping_ids = []
    for banded_range in banded_ranges:
        if banded_range["range"]["sheetId"] == sheet_id:
            band_start_row = banded_range["range"]["startRowIndex"]
            band_end_row = banded_range["range"]["endRowIndex"]
            band_start_col = banded_range["range"]["startColumnIndex"]
            band_end_col = banded_range["range"]["endColumnIndex"]
            if not (
                band_end_row <= start_row
                or band_start_row >= end_row
                or band_end_col <= start_col
                or band_start_col >= end_col
            ):
