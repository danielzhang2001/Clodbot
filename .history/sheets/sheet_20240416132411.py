...
def get_players(values: List[List[str]]) -> List[str]:
    ...
    for i in range(0, len(values), 15):
        if i < len(values):
            names.extend([name for name in values[i] if name != ""])
    return names
...
def get_pokemon(values: List[List[str]]) -> List[str]:
    ...
    pokemon = []
    # Start at the third row, and increase by 15 after processing each block (12 rows processed + 3 rows skipped)
    for i in range(2, len(values), 15):  # Starts at index 2, skips 15 rows after each 12-row block
        # Process the next 12 rows from the starting index i
        for j in range(i, min(i + 12, len(values))):
            row = values[j]
            # Extend pokemon list with entries that are non-empty and not purely numeric
            pokemon.extend([
                name for name in row if name and not (name.replace('.', '', 1).isdigit() and name.count('.') <= 1)
            ])
