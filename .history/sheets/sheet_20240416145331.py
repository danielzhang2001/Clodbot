...
def get_pokemon(values: List[List[str]]) -> List[str]:
    ...
    for i in range(2, len(values), 15):  # Start at the third row, skip 15 rows after each 12-row block
        for j in range(i, min(i + 12, len(values))):
            row = values[j]
            pokemon.update(
                name.strip()  # Remove leading/trailing whitespace
                for name in row
                if name.strip()  # Ensure name is not empty or whitespace
                and not (name.strip().replace(".", "", 1).isdigit() and name.strip().count(".") <= 1)
            )
