...
def update_data(
    ...
    pokemon = [data[0] for data in pokemon_data]
    update_requests = []
    insert_requests = []
    base_range, row_range = range.split("!")
    start_row, _ = row_range[1:].split(":")
    start_row_index = int(start_row)
    pokemon_index = {name: idx for idx, name in enumerate(pokemon)}
    current_max_row = start_row_index + len(pokemon) - 1
...
def check_labels(values: List[List[str]], name: str) -> bool:
    ...
    for row_index, row in enumerate(values):
        try:
            ...
            if (name in row and row_index + 1 < len(values) and values[row_index + 1][name_index] == "Pokemon" and values[row_index + 1][name_index + 1] == "Kills" and values[row_index + 1][name_index + 2] == "Deaths"):
                return True
    return False
