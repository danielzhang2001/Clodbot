...
def update_pokemon(
    ...
    updated_values = [
    [
        values[0][0],  # Pokemon Name
        str(int(values[0][1]) + 1),  # Games, incremented by 1
        str(int(values[0][2]) + stats[0]),  # Kills, incremented by stats[0]
        str(int(values[0][3]) + stats[1]),  # Deaths, incremented by stats[1]
    ]
]
    print("a")
    service.spreadsheets().values().update(
