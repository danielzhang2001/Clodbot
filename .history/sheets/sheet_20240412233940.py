...
def bold_cells(
    ...
    # Bolds all the cells for a player's section for formatting purposes.
    _, cell_range = cell_range.split("!")
    start_cell, end_cell = cell_range.split(":")
...
def color_cells(
    ...
    # Colors all the cells for a player's section for formatting purposes.
    _, range_part = cell_range.split("!")
    start_cell, end_cell = range_part.split(":")
    start_row_index = int("".join(filter(str.isdigit, start_cell))) - 1
    end_row_index = int("".join(filter(str.isdigit, end_cell)))
    start_column_index = ord(start_cell[0]) - ord("A")
    end_column_index = (
        ord(end_cell[0]) - ord("A") + 1
    )  # We assume the range is within one column letter

    # If your range spans across multiple columns, adjust start_column_index and end_column_index accordingly
    body = {
        "requests": [
            {
                "addBanding": {
...
def color_text(
    ...
    # Colors all the text for a player's section white for formatting purposes.
    # Colors all the cells for a player's section for formatting purposes.
    _, cell_range = cell_range.split("!")
    start_cell, end_cell = cell_range.split(":")
    start_col = "".join(filter(str.isalpha, start_cell))
