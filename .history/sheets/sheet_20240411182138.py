...
def update_pokemon(
    ...
    """Updates existing Pokemon stats in the Google Sheet by adding new stats to current stats."""
    row_index = (
        current_values.index(pokemon_name) + start_row
    )  # Assumes `pokemon_name` directly corresponds to row index calculation
    ...
    update_range = f"{sheet_name}!{start_col}{row_index}:{end_col}{row_index}"
    updates.append({"range": update_range, "values": [[updated_kills, updated_deaths]]})
