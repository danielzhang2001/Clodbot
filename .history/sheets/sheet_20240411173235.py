...
def update_data(
    ...
    current_values = current_result.get(
    ...
    for pokemon_name, new_stats in pokemon_data:
        if pokemon_name in current_pokemon:
            row_index = current_pokemon[pokemon_name] + start_row
            print("BEFORE")
            current_kills = safe_int(current_values[row_index - start_row][1] if len(current_values[row_index - start_row]) > 1 else 0)
            current_deaths = safe_int(current_values[row_index - start_row][2] if len(current_values[row_index - start_row]) > 2 else 0)
            print("AFTER")
            ...
            updates.append(
                {"range": update_range, "values": [[updated_kills, updated_deaths]]}
            )
        else:
            end_row = add_pokemon(
                sheet_name,
...
def update_pokemon(
    ...
    # Update Pokemon stats based on existing Pokemon in the sheet.
    row_index = current_pokemon[pokemon_name] + start_row
    current_kills, current_deaths = list(
        map(int, current_values[row_index - start_row][1:3])
    )
    new_kills, new_deaths = new_stats
