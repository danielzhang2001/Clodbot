...
def update_data(
    ...
    pokemon = {row[0]: idx for idx, row in enumerate(rows) if row}
    ...
    for pokemon_name, stats in pokemon_data:
        if pokemon_name in pokemon:
            ...
            row_range = f"Stats!{start_col}{row_index}:{start_col[-1]}{row_index}"
            update_pokemon(
                service,
                spreadsheet_id,
                row_range,
    ...
    empty_row_index = next(
        (start_row + idx for idx, row in enumerate(existing_rows) if not row),
        end_row + 1,
    )
    empty_row_range = (
        f"Stats!{start_col}{empty_row_index}:{start_col[-1]}{empty_row_index}"
    )
    for pokemon_name, stats in pokemon_data:
        if pokemon_name not in current_pokemon:
            add_pokemon(
                service,
                spreadsheet_id,
                empty_row_range,
...
def update_pokemon(
    ...
    current_values = result.get("values", [[0, 0, 0, 0]])[0]
    updated_games = int(current_values[1]) + 1
    updated_kills = int(current_values[2]) + stats[0]
    updated_deaths = int(current_values[3]) + stats[1]
    updated_values = [[current_values[0], updated_games, updated_kills, updated_deaths]]
    service.spreadsheets().values().update(
        spreadsheetId=spreadsheet_id,
        range=row_range,
