...
def update_data(
    ...
    start_range, end_range = (cell_range.split("!")).split(":")
...
def update_pokemon(
    ...
    range = f"{sheet_name}!{col_range}{row}"
    # Get current values
    result = service.spreadsheets().values().get(spreadsheetId=spreadsheet_id, range=range).execute()
    current_values = result.get('values', [[0, 0, 0]])[0]  # Default to 0 if not found

    # Calculate updated values
    updated_games = int(current_values[1]) + 1  # Assuming the first column within the range is Games
    updated_kills = int(current_values[2]) + stats[0]
    updated_deaths = int(current_values[3]) + stats[1]

    # Update the row with the new Pokémon statistics
    updated_values = [[current_values[0], updated_games, updated_kills, updated_deaths]]
    service.spreadsheets().values().update(spreadsheetId=spreadsheet_id, range=range, body={'values': updated_values}, valueInputOption='USER_ENTERED').execute()
