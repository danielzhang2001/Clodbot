...
def update_pokemon(
    ...
    games = int(current_data[0][0]) + 1
    kills = int(current_data[0][1]) + stats[0]
    current_deaths = int(current_data[0][2]) + stats[1]
    values = {"values": [[current_games, current_kills, current_deaths]]}
    service.spreadsheets().values().update(
        spreadsheetId=spreadsheet_id,
        range=range,
        body=values,
