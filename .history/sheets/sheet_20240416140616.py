...
def create_player_message(values: List[List[str]]) -> str:
    #
    player_names = get_players(values)
    message = "Player Names:\n\n```"  # Start of the message and code block
    message += "\n".join(player_names)  # Add each player name on a new line
        ...
        for j in range(i, min(i + 12, len(values))):
            ...
            pokemon.extend(
                [
                    ...
                    if name
                    and not (
                        name.replace(".", "", 1).isdigit() and name.count(".") <= 1
                    )
                ]
            )
    return pokemon
